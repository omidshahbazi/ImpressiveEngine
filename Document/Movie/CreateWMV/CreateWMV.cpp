// CreateWMV.cpp : Defines the entry point for the application.
//
...
///
CwmvFile	wmvFile("Output.wmv", WMProfile_V80_384Video, 1);;
                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_CREATEWMV, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, (LPCTSTR)IDC_CREATEWMV);
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, (LPCTSTR)IDI_CREATEWMV);
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= (LPCTSTR)IDC_CREATEWMV;
	...
	case WM_PAINT:
		{
			...
			DrawText(hBackDC, str, (int)strlen(str), &rect, DT_VCENTER | DT_SINGLELINE | DT_CENTER);
			...
			if(FAILED(wmvFile.AppendNewFrame(320,240,pBits)))//	wmvFile.AppendNewFrame(hBackBitmap)))
			{
				MessageBox(hWnd, wmvFile.GetLastErrorMessage(), _T("Error Occured"), MB_OK|MB_ICONERROR);
				DestroyWindow(hWnd);
			}
...
void EnumProfiles()
{
	...
	if(FAILED(pProfileManager2->SetSystemProfileVersion(WMT_VER_7_0)))
	{
